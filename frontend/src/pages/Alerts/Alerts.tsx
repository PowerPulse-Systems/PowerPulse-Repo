    ...
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl shadow-sm overflow-hidden">
        <div className="p-4 border-b border-slate-800 bg-slate-800/30">
          <h2 className="font-semibold text-slate-100">Active Alerts</h2>
        </div>
        <div className="divide-y divide-slate-800">
          {alerts.map(alert => (
            <div key={alert.id} className="p-4 flex items-start space-x-4 hover:bg-slate-800/20">
              <div className="mt-1">
              ...
              <div className="flex-1">
                <div className="flex justify-between">
                  <h3 className="font-semibold text-slate-200">{alert.type} - {alert.source}</h3>
                  <span className="text-xs text-slate-500">{alert.time}</span>
                </div>
                <p className="text-sm text-slate-400 mt-1">{alert.desc}</p>
                <div className="mt-3 space-x-2">
                  <button className="px-3 py-1 bg-slate-800 border border-slate-700 text-xs rounded hover:bg-slate-700">Acknowledge</button>
      ...
      <div className="bg-slate-900 border border-slate-800 p-6 rounded-xl shadow-sm">
        <h2 className="text-lg font-semibold text-slate-100 mb-4">Threshold Configuration</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="p-4 border border-slate-800 rounded bg-slate-950">
            <div className="flex justify-between mb-2">
              <span className="text-sm text-slate-300">Global Overcurrent Limit</span>
              <span className="text-sm text-slate-500">50 A</span>
            </div>
            <input type="range" className="w-full accent-blue-500" />
          </div>
          <div className="p-4 border border-slate-800 rounded bg-slate-950">
            <div className="flex justify-between mb-2">
              <span className="text-sm text-slate-300">Voltage Drop Alert</span>
              <span className="text-sm text-slate-500">&lt; 200 V</span>
            </div>
            <input type="range" className="w-full accent-blue-500" />
          </div>
        </div>
